...
export default function MyReports() {
  ...
  const loadReports = useCallback(async () => {
    if (!userId) {
      console.log("[Retailer MyReports] No userId available:", { 
        user: authState.user,
        userId 
      });
      setLoading(false);
    ...
    try {
      setLoading(true);
      console.log("[Retailer MyReports] Fetching reports for userId:", userId, {
        type: "submitted",
      ...
      console.log("[Retailer MyReports] Successfully fetched reports:", data.length);
      
      // Ensure data is an array
      if (!Array.isArray(data)) {
        console.error("[Retailer MyReports] Invalid response format - expected array, got:", typeof data, data);
      ...
      console.error("[Retailer MyReports] Failed to load reports:", error);
      console.error("[Retailer MyReports] Error details:", {
      ...
      Alert.alert(
        "Error", 
        error?.message || error?.response?.data?.message || "Failed to load reports"
      );
    } finally {
      setLoading(false);
    }
  }, [userId, currentPage, authState.user]);

  useEffect(() => {
    if (userId) {
      loadReports();
    } else {
      // If userId is not available, stop loading
      setLoading(false);
    }
